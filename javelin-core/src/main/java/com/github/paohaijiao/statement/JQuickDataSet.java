...
public class JQuickDataSet {
    ...
    public List<JQuickColumnMeta> getColumns() {
        return columns;
    }
    ...
    public List<String> getColumnNames() {
        return columns.stream().map(JQuickColumnMeta::getName).collect(Collectors.toList());
    }
     ...
     */
    public JQuickDataSet subDataset(int startIndex, int endIndex) {
        List<JQuickRow> subRows = rows.subList(startIndex, Math.min(endIndex, rows.size()));
        return new JQuickDataSet(columns, new ArrayList<>(subRows));
    }
    /**
     ...
     */
    public JQuickDataSet limit(int n) {
     ...
     */
    public JQuickDataSet skip(int n) {
     ...
     */
    public JQuickDataSet orderBy(String columnName) {
     ...
     * @return a new sorted dataset
     */
    public JQuickDataSet orderBy(String columnName, boolean ascending) {
        ...
        return new JQuickDataSet(columns, sorted);
    }
    /**
     ...
     * @return a new sorted dataset
     */
    public JQuickDataSet orderBy(String... columnNames) {
        List<JQuickRow> sorted = new ArrayList<>(rows);
     ...
     */
    public JQuickDataSet distinct() {
        List<JQuickRow> distinct = rows.stream()
                .distinct()
                .collect(Collectors.toList());
        return new JQuickDataSet(columns, distinct);
    }
    /**
     ...
     */
    public JQuickDataSet distinctBy(String... columnNames) {
     ...
     */
    public JQuickDataSet concat(JQuickDataSet other) {
        List<JQuickRow> combined = new ArrayList<>(rows);
        combined.addAll(other.getRows());
        return new JQuickDataSet(columns, combined);
    }
    /**
     ...
     */
    public JQuickDataSet innerJoin(JQuickDataSet other, String thisColumn, String otherColumn) {
        ...
        List<JQuickColumnMeta> mergedColumns = new ArrayList<>(columns);
     ...
     */
    public JQuickDataSet addColumn(String newColumnName, Class<?> type, String source, Function<JQuickRow, Object> valueExtractor) {
        ...
        List<JQuickColumnMeta> newColumns = new ArrayList<>(columns);
        newColumns.add(new JQuickColumnMeta(newColumnName, type, source));
     ...
     */
    public JQuickDataSet select(String... columnNames) {
        Set<String> keepSet = new HashSet<>(Arrays.asList(columnNames));
        List<JQuickColumnMeta> selectedColumns = columns.stream()
        ...
        List<JQuickRow> selectedRows = rows.stream()
                ...
                .collect(Collectors.toList());
        return new JQuickDataSet(selectedColumns, selectedRows);
    }
    /**
     ...
     */
    public JQuickDataSet drop(String... columnNames) {
        Set<String> dropSet = new HashSet<>(Arrays.asList(columnNames));
        List<JQuickColumnMeta> remainingColumns = columns.stream()
        ...
        List<JQuickRow> remainingRows = rows.stream()
                ...
                .collect(Collectors.toList());
        return new JQuickDataSet(remainingColumns, remainingRows);
    }
    /**
    ...
    public JQuickDataSet filter(Predicate<JQuickRow> predicate) {
        List<JQuickRow> filtered = rows.stream().filter(predicate).collect(Collectors.toList());
        return new JQuickDataSet(columns, filtered);
    }

    public JQuickDataSet map(Function<JQuickRow, JQuickRow> mapper) {
        List<JQuickRow> mapped = rows.stream().map(mapper).collect(Collectors.toList());
        return new JQuickDataSet(columns, mapped);
    }
